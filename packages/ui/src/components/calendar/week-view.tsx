  ...
  const processedDayEvents = useMemo(() => {
    const result = days.map(day => {
      ...
      sortedEvents.forEach(event => {
        ...
        const adjustedStart =
          isSameDay(day, eventStart) ? eventStart : dayStart;
        const adjustedEnd =
          isSameDay(day, eventEnd) ? eventEnd : addHours(dayStart, 24);
    ...
    <div data-slot="week-view" className="flex h-full flex-col">
      <div className="bg-background/80 border-border/70 sticky top-0 z-30 grid grid-cols-8 border-b backdrop-blur-md">
        ...
        {days.map(day => (
          <div
            key={day.toString()}
            className="data-today:text-foreground text-muted-foreground/70 data-today:font-medium py-2 text-center text-sm"
